...
SELECT t.id, t.created_at, t.is_blocked, t.name, t.tier, t.email, t.is_banned, t.blocked_reason, t.cluster_id, t.sandbox_scheduling_labels, t.ory_organization_id, t.slug, tl.id, tl.max_length_hours, tl.concurrent_sandboxes, tl.concurrent_template_builds, tl.max_vcpu, tl.max_ram_mb, tl.disk_mb FROM "public"."team_api_keys" tak
...
SELECT t.id, t.created_at, t.is_blocked, t.name, t.tier, t.email, t.is_banned, t.blocked_reason, t.cluster_id, t.sandbox_scheduling_labels, t.ory_organization_id, t.slug, tl.id, tl.max_length_hours, tl.concurrent_sandboxes, tl.concurrent_template_builds, tl.max_vcpu, tl.max_ram_mb, tl.disk_mb
FROM "public"."teams" t
...
const getTeamsByOryOrganizationID = `-- name: GetTeamsByOryOrganizationID :many
SELECT id, created_at, is_blocked, name, tier, email, is_banned, blocked_reason, cluster_id, sandbox_scheduling_labels, ory_organization_id, slug FROM "public"."teams"
WHERE ory_organization_id = $1::uuid
ORDER BY created_at ASC
`

func (q *Queries) GetTeamsByOryOrganizationID(ctx context.Context, oryOrganizationID uuid.UUID) ([]Team, error) {
	rows, err := q.db.Query(ctx, getTeamsByOryOrganizationID, oryOrganizationID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT t.id, t.created_at, t.is_blocked, t.name, t.tier, t.email, t.is_banned, t.blocked_reason, t.cluster_id, t.sandbox_scheduling_labels, t.ory_organization_id, t.slug, ut.is_default, tl.id, tl.max_length_hours, tl.concurrent_sandboxes, tl.concurrent_template_builds, tl.max_vcpu, tl.max_ram_mb, tl.disk_mb
FROM "public"."teams" t
         JOIN "public"."users_teams" ut ON ut.team_id = t.id
         JOIN "public"."team_limits" tl on tl.id = t.id
WHERE ut.user_id = $1
